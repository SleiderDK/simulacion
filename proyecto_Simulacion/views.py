...
def HolaMundo(request):
   ...
   return render(request, "miplantilla.html", {"nombre":nombre,"fecha":fecha_actual,"lista":lista})
...
def Inicio(request):
   ...
   return render(request, "index.html", {'nombre':nombre,'fecha':fecha_actual})
...
def cuadradosMedios(request):  
    ...
    dfrac = df["Xi"]/10**l
    df["ri"] = dfrac
    html = df.to_html(classes='table table-sm table-secondary')  
    x1=df['ri']
    plt.plot(x1)
    plt.title("Grafico")
    plt.xlabel("Serie")
...
def promedioMovil(request):
    ...
    lista2 = llenarDatosVacios(lista) 
...
def alisamientoExponencial(request):
    ...
    lista3,error = [],[]
    j = 0
    while j < num_Element+1:
        if j == 0:
            lista3.append(lista2[0])
            error.append(lista2[j] - lista3[j])
            j = j + 1
        else:
            sn1 = alfa*lista2[j-1]
...
def regresionLineal(request):
    listaX = request.POST["listaX"]
    listaY = request.POST["listaY"]
    z = float(request.POST["predecir"])
    liX = llenarDatosVacios(listaX)
    liY = llenarDatosVacios(listaY)
    ...
    html = df3.to_html(classes='table table-sm table-secondary')
...
def lineaDeEsperaMontecarlo(request):
    ...
    tablaLlegada = df.to_html(classes='table table-sm table-secondary')
    ...
    tablaServicio = df2.to_html(classes='table table-sm table-secondary')
